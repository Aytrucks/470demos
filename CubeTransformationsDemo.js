...
window.onload = function init() {
	...
	// Put colored points on the blue face that is visible in demo
	// distinct color for each vertex-shader
	...
	document.getElementById("ButtonRT").onclick = function(){
		mvMatrix = mult(rotate(60.0, 15.0, 15.0, 1.0),translate(0.5, 0.5, 0.5));
...
var render = function() {
		...
		gl.drawArrays( gl.POINTS, numVertices, 4);
		
		// draw the origin
		mvMatrix2 = mat4();
        
        
        mvMatrix = mult(translate(0.5,0.5,0.5), rotate(theta, 0.5, 1.0, 0.0));
        ...
        gl.drawArrays( gl.TRIANGLES, 0, numVertices );
        mvMatrix2 = mult(translate(-0.5,-0.5,-0.5), rotate(theta, 0.5, 1.0, 0.0));
        mvMatrix2 = mult(mvMatrix, scalem(0.5,0.5,0.5));
		gl.drawArrays( gl.POINTS, numVertices+4, 1 );
